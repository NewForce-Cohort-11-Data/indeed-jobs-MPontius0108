...
-- 3.	How many postings are in Tennessee? How many are there in either Tennessee or Kentucky? - TN = 21, KY = 6

SELECT
	COUNT(CASE WHEN location = 'TN' THEN location END) AS TN_Jobs,
	COUNT(CASE WHEN location = 'KY' THEN location END) AS KY_Jobs
FROM
	data_analyst_jobs;
...
-- 6.	Show the average star rating for companies in each state. The output should show the state as `state` and the average rating for the state as `avg_rating`. Which state shows the highest average rating? - NE (Nebraska)
...
SELECT 
	COUNT(DISTINCT title) AS CA_unique_jobs
FROM
	data_analyst_jobs
WHERE location ='CA';
...
-- 10.	Add the code to order the query in #9 from highest to lowest average star rating. Which company with more than 5000 reviews across all locations in the dataset has the highest star rating? What is that rating? - Unilever - 4.2

SELECT
	company,
	ROUND(AVG(star_rating), 4) AS avg_rating
FROM
	data_analyst_jobs
WHERE review_count > 5000
	AND company IS NOT NULL
...
SELECT domain AS industry,
	COUNT(title) AS job_title_count
FROM
	data_analyst_jobs
